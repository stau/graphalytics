 ...
 * @author Wing Ngai
 */
public class UndirectedCommunityDetectionComputation extends NewUndirectedVertex<LongWritable, CDLabel, NullWritable, Text> {
    ...
    private void propagateLabel() {
        ...
        for (LongWritable neighbour : getNeighbors()) {
            Text initMessage = new Text(getId() + "," + cd.getLabelName() + "," + cd.getLabelScore() + "," +
		            neighboursSize());
    ...
    @Override
    public void loadPUStruct(LudographPUFactory<LongWritable, CDLabel, NullWritable, Text> ludographPUFactory, DataInput dataInput) throws IOException, InstantiationException, IllegalAccessException {
        // not implemented
    }
