...
/**
 * UndirectedLocalClusteringCoefficientComputation based on Tim Hegeman's implementation
 *
 * @author Sietse T. Au
 */
public class UndirectedLocalClusteringCoefficientComputation extends
		NewUndirectedVertex<LongWritable, DoubleWritable, NullWritable, LocalClusteringCoefficientMessage> {
	...
	public void compute(Iterable<LocalClusteringCoefficientMessage> messages) {
		if (getSuperStep() == 1) {
			...
			double lcc = computeLCC(neighboursSize(), messages);
			getValue().set(lcc);
	...
	@Override
	public void loadPUStruct(LudographPUFactory<LongWritable, DoubleWritable, NullWritable, LocalClusteringCoefficientMessage> ludographPUFactory, DataInput dataInput) throws IOException, InstantiationException, IllegalAccessException {
